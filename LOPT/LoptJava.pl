...
# Version 2.1 (first Perl version)
use strict;
require 'InvertMatrixJava.pl';
require 'vacair.pl';
...
#      http://physics.nist.gov/cuu/Constants/index.html
        ...
        # to all 'fixed' levels. Its value is set to $FixWeightFactor times the
        # smallest uncertainty of observed transitions and fixed levels
my $FixedLevUncert;
my $FixedLevWeight;
my $FixWeightFactor = 0.1; # Weight used to effectively fix the "fixed" levels.
...
my (@Quest1,@Quest2, @Sigma, @FixLevs,@Lines,@Levels,@GroupLines,@CGroups,
  ...
  $TransFilename,$FixLevFilename,$LevOutFilename,$TransOutFilename);
my $MaxCGNo = 0;
my %Widths = ();
...
sub ReadLines {
  ...
  while (($S = <Tfil>) && defined($S)) {
    ...
    my $precision = ($Res) ? 0 : "1e-$num_decimals" / $lambda;
...
############################################################################
  ...
  foreach my $L (@{$G}) {
    ...
    next if $T_max->{'FromLevel'}->{'Fixed'};
    next if $T_max->{'ToLevel'}->{'Fixed'};
    ...
    if ( $max_weight/$min_weight > $threshold ) {
      ...
      if ( (!defined($L_from->{'Subst'}) || ($L_from->{'Subst'}->[1] < $max_weight)) &&
        (!defined($L_from->{'max_weight'}) ||
          (!$L_from->{'max_weight'}->[0]->{'ToLevel'}->{'Fixed'}) ||
        ...
        if ( $weight/$min_weight > $threshold) {
          ...
          if ( (!defined($T->{'FromLevel'}->{'Subst'}) || ($T->{'FromLevel'}->{'Subst'}->[1] < $weight)) &&
            (!defined($T->{'FromLevel'}->{'max_weight'}) ||
              (!$T->{'FromLevel'}->{'max_weight'}->[0]->{'ToLevel'}->{'Fixed'}) ||
...
sub FindLevels($$$) {
  ...
  if ( $Pass == 1 ) {
    ...
    for ( my $k = 0; $k <= 2*$statistics_size; $k++ ) {
      ...
      my ($L,$piv) = $use_Cholesky ? CholeskyDecomposition($A_scaled) : LUDecomposition($A_scaled);
  ...
  for (my $i = 1; $i <= $#GroupLines + 1; $i++) {
    ...
    if (trans_SingleLineLevelFlag($PT) ne '*') {
      ...
      if ( ($Dc <= $PT->{'WnUnc'}) || ($PT->{'Flags'} & $FMP) ) {
        ...
        # but assume that it can not exceed the measured line uncertainty.
...
sub RoundLevel($) {
  ...
  ($PL->{'Er'},$min_unc) = round($PL->{'E'}+$PL->{'Shift'},$PL->{$min_unc},$LevRoundThreshold*$factor,1);
    # now $PL->{$min_unc} accounts for rounding error in $PL->{'E'}

    # Upper bound of error due to limited floating-point precision
  ($PL->{'dr'},$min_unc) = round($PL->{'d'},$min_unc,$LevRoundThreshold*$factor,0);
}
...
sub RoundLevels() {
  ...
  for (my $i = 0; $i <= $#Levels; $i++) {
    ...
    for (my $k = 0; $k <= $#UncBases; $k++) {
      my $x;
      ($x, $PL->{'UncRelBases'}->[$k]) = round($PL->{'UncRelBases'}->[$k],$PL->{'UncRelBases'}->[$k],$LevRoundThreshold,0);
    }
  }
  $d_max = $d_max;
} # RoundLevels
  ...
  for (my $i = 1; $i <= $#Levels + 1; $i++) {
    ...
    if (!$PL->{'Fixed'}) {
      ...
      if ($PL->{'NT2'} > 1) {
        ...
        ($PL->{'D1r'},$Dc) = round($PL->{'D1'},$PL->{'D1'},$LevRoundThreshold,0);
        if ($PL->{'GNo'}>1) {
          $PL->{'D2'} = sqrt(sqr($PL->{'D2'}) + $Dc);
          ($PL->{'D2r'},$Dc) = round($PL->{'D2'},$PL->{'D2'},$LevRoundThreshold,0);
        }
      }
    } else {
      if ($PrintUncCorr) {
